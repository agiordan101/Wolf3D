...
/*   Updated: 2019/03/17 14:58:07 by gmonacho    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
int				parser(int const fd, t_map *map, t_player *player)
{
	...
	{
		map->len_x = (map->len_y == 0) ? ft_countnumbers(line) : map->len_x;
		if (ft_countnumbers(line) != map->len_x && map->len_y > 0)
			return (-5);
		if ((error = convert_int(line, &int_line, map->len_x)) <= 0)
			return (error);
		tmp = map->tab;
		if (!(map->tab = ft_addinttabn(map->tab, map->len_y,
										map->len_x, int_line)))
			return (-2);
